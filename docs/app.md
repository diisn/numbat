...
| TCP Server | `transport.NewServer(host:port)` | 设置 Bus、runsDir、trace，注册 RPC handler |
| HTTP Gateway | `transport.NewGateway(host:gatewayPort)` | `/health` `/metrics` `/ws`；`SetRPCServer(server)` 复用同一套 RPC dispatch |
...
`App.Run` 并行启动服务：TCP/HTTP 两个 server 的错误经 `errCh` 汇总（任一返回即退出）；配置了启用通道时额外启动 Channel Manager：
...
go func() { errCh <- server.Run(ctx) }()       // TCP 7437
go func() { errCh <- gateway.Run(ctx) }()      // HTTP/WS 7438
select {
case err := <-errCh: // 任一 server 出错即退出
case <-ctx.Done():   // 信号取消，优雅关闭
}
```

监听 SIGINT/SIGTERM → 取消 context → 各组件优雅关闭（Server 关监听等 in-flight、网关 `Shutdown`、Channel Disconnect、关 MCP 子进程、刷 trace）。
